...
# ⚠️ 데이터 누수 방지:
# 타겟 인코딩 통계는 "시간 기반 Train 구간"에서만 계산해야 함.
# 여기서는 학습데이터여부 == 'Y' 중에서 취득일자를 기준으로 앞선 70%를 Train으로 사용.
date_col = '취득일자'
df_train_source_te = df_final[df_final['학습데이터여부'] == 'Y'].copy()
df_train_source_te = df_train_source_te.sort_values(date_col)
n_train_pool = len(df_train_source_te)
train_end_idx = int(n_train_pool * 0.7)
train_indices_te = df_train_source_te.iloc[:train_end_idx].index
for col in categorical_cols:
    df_final[col] = df_final[col].fillna('Unknown').astype(str)
    # 시간 기반 Train 인덱스에서만 각 카테고리별 평균 수명 및 빈도수 계산
    train_mask_te = df_final.index.isin(train_indices_te)
    target_mean = df_final[train_mask_te].groupby(col)['실제수명'].mean()
    category_counts = df_final[train_mask_te][col].value_counts()
