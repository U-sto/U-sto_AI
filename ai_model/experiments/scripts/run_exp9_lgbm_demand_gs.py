...
from sklearn.model_selection import ParameterGrid # GridSearch 탐색용
...
RUN_DIR = EXPERIMENTS_DIR / "runs" / f"{RUN_ID}_exp9_lgbm_demand_gs"
VALID_RESULT_PATH = TABLES_DIR / "exp9_lgbm_demand_gs_results.csv"
TOP_TEST_PATH = TABLES_DIR / "exp9_lgbm_demand_gs_top_test.csv"
...
def main() -> None:
    ...
    valid_rows = []
    print(" GridSearchCV 탐색 루프 진입 중...")
    
    for idx, params in enumerate(grid_params):
        t0 = time.perf_counter()
        # 고정 7개 피처 세트(FEATURES) 적용
        ...
        valid_rows.append(
            {
                "model": "LightGBM",
                "variant": f"lgbm_gs_variant_{idx}",
    ...
    # Validation 성능이 가장 좋았던 조합들을 Train+Valid 병합 데이터로 최종 테스트 진행
    ...
    # 기존 요약 파일이 존재하면 병합(누적), 없으면 새로 생성
    if SUMMARY_PATH.exists():
        try:
            existing_summary = pd.read_csv(SUMMARY_PATH)
            # 중복 데이터 방지를 위해 현재 모델 기록 제거 후 병합
    ...
    print("Stage3 monthly model search (LGBM GridSearch) complete")
