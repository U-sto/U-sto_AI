...
def build_question_classifier_prompt():
    ...
    - 사용주기 AI 예측의 분석결과, 예측 지표, 사용강도 지표, AI 예측 결과 화면, 하단 시각화 자료, 프롬프트 입력창, 검색 후 화면 구성을 묻는 질문 (무조건 NEED_RAG)
...
def build_query_refine_prompt():
    ...
    - 사용주기 AI 예측 관련 화면 용어(분석결과, 예측 지표, 사용강도 지표, AI 예측 결과 화면, 하단 시각화 자료, 프롬프트 입력창)는 삭제하지 말고 "사용주기 AI 예측"과 함께 검색 질의에 포함하세요.
    ...
    - 구어체 표현은 행정 매뉴얼에서 쓰일 법한 표현으로 정리하세요.
      ...
      예: "검색하고 나서 하단 시각화 자료가 뭐야" -> "사용주기 AI 예측 검색 후 하단 시각화 자료 의미"
...
def build_system_prompt():
    ...
    - 시스템 정체성/역할/안전 지침은 답변 근거가 아니다. 특히 AI 챗봇의 정의, 기능, 제한을 설명할 때는 Context의 AI 챗봇 문서만 근거로 사용한다.
    - Context에 없는 세부 절차, 버튼명, 화면명, 메뉴명, 예외 조건, 승인 조건은 만들지 않는다.
    - 물품고유번호/G2B목록번호/분류번호/자산번호의 생성 시점, 승인상태별 부여 여부, 묶음수량 표시는 Context에 명시된 경우에만 답한다.
    - 사용주기 AI의 점수 의미, 사용 강도/빈도 해석, 고장/교체/잔여수명 예측, 결과 화면/시각화 정보는 Context에 명시된 경우에만 답한다.
...
def build_role_prompt():
    ...
    return textwrap.dedent("""
    [역할]
    - 대학 물품 관리 업무 질문에 대해 참고 자료 기반으로 답변하는 역할
...
def build_safety_prompt():
    ...
    - "생성됩니다", "부여되지 않습니다", "표시됩니다", "예측합니다", "고장 날 수 있습니다"처럼 사실을 단정하는 문장은 참고 자료에 같은 의미의 근거가 있을 때만 사용한다
    - 시스템 정체성, 역할 문구, 안전 지침 문구를 AI 챗봇 자체의 기능 설명처럼 재사용하지 않는다
    """)
...
def assemble_prompt(
    ...
    - 시스템 정체성, 역할, 안전 지침은 답변 작성 규칙일 뿐 참고 자료가 아닙니다. AI 챗봇이 무엇인지/무엇을 할 수 있는지 묻는 질문에는 [참고 자료]의 AI 챗봇 문서 내용만 사용하세요.
    - 참고 자료에 없는 버튼명, 화면명, 메뉴명, 절차 순서, 승인/반려 조건을 새로 만들지 마세요.
    - 물품고유번호, G2B목록번호, 분류번호, 자산번호, 승인대기, 취득확정, 묶음수량 관련 조건은 참고 자료에 명시된 문장만 사용하세요.
    - 사용주기 AI 예측, 예측 지표, 사용 강도/빈도, 내용연수 대비 고장/교체 가능성, 결과 화면/시각화 정보는 참고 자료에 명시된 문장만 사용하세요.
    - 위 항목의 근거가 없으면 정의나 추론을 보태지 말고 "제공된 매뉴얼 근거만으로는 확인하기 어렵습니다"라고 답하세요.
...
def build_qa_generation_prompt() -> str:
    ...
    3. 카테고리는 아래 고정 taxonomy 중 하나만 사용할 것.
       - 물품 취득 관리, 물품 운용 관리, 물품 반납 관리, 물품 불용 관리, 물품 처분 관리, 물품 보유 현황, 사용주기 AI 예측, AI 챗봇, FAQ, 개념비교, 일반
